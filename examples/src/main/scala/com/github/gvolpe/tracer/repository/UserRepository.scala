...
class DefaultUserRepository[F[_]](implicit F: Sync[F], L: TracerLog[Traceable[F, ?]]) extends UserRepository[Traceable[F, ?]] {
  private val users = scala.collection.mutable.Map.empty[Username, User]

  override def find(username: Username): Traceable[F, Option[User]] = Kleisli { id =>
    for {
